...
function estimatepi2(n)
	4 / n * count(x -> x ≤ 1.0, sum(rand(n, 2).^2, dims=2))
end
...
# ╔═╡ 07887839-7fca-441f-9f51-e2be9ff8d640
md"""
λ:
$(@bind λ html"<input type=range min=0.0 max=300.0>")
...
# ╔═╡ e62b2e8a-417f-459b-ab74-7bf1db10aa4b
begin
	using Plots
	...
	sc = scatter(t,Y₁; label="Dolphins", color=blue,
	  ylabel="IQ (-)", xlabel ="Time (year BC)", legend=:topleft)
	sc = scatter!(sc, t,Y₂; label="Humans", color=red)	
	sc = plot!(sc, t, Yₚ₁, color=blue, linewidth=5)
	plot!(sc, t, Yₚ₂, color=red, linewidth=5)
end
...
manifest_format = "2.0"
project_hash = "99c3e72d4eac677abfbe8ddb6c7c66214ac5be88"
