...
[ (a1,a2,interact(a1,a2)) for a1 in fauna, a2 in fauna]
...
begin
	struct Vandermonde{T} <: AbstractMatrix{T}
		α::AbstractVector{T}
		m::Int
		Vandermonde(α::AbstractVector{T}, m) where {T} =
			new{T}(α,m)
	end
...
function Base.getindex(V::Vandermonde, i, j)
	(n,m) = size(V)
	@assert 1 ≤ i ≤ n
	@assert 1 ≤ j ≤ m
	V.α[i]^(j-1)
...
# ╔═╡ 0d9d3056-d52b-4396-98bb-895c8c34b0ac
size(V)
...
# scalar scaling
Base.:*(a::Real, c::RGB) = RGB(a*c.r, a*c.b, a*c.b)
...
struct WizCur
	...
	function WizCur(galleons::Int,sickles::Int,knuts::Int)
		total_knuts = 493galleons + 29sickles + knuts
		g = total_knuts ÷ 493
		remaining = total_knuts % 493
		s = remaining ÷ 29
		k = remaining % 29
		new(g,s,k)
	end
...
# ╔═╡ 8277565a-655b-47b1-9b2f-2ad2b2adc6e5
WizCur(0,0,0)

# ╔═╡ 48301af2-5d94-11eb-0019-7737667c9cea
galleons(money::WizCur) = money.galleons # complete me!

# ╔═╡ 4ea80eda-5d94-11eb-3882-21a41d2d65f8
sickles(money::WizCur) = money.sickles # complete me!

# ╔═╡ 5af60d90-5d94-11eb-2ee4-b7bfc2caf53b
knuts(money::WizCur) = money.knuts # complete me!

# ╔═╡ 5f7c75ac-5d94-11eb-137a-7914cd009821
function Base.show(io::IO, money::WizCur)
    print(io, "G$(galleons(money)) S$(sickles(money)) K$(knuts(money))")
end

# ╔═╡ 95146d46-5d94-11eb-22aa-c1a544e0d784
Base.:+(m1::WizCur, m2::WizCur) = WizCur(m1.galleons + m2.galleons, m1.sickles + m2.sickles, m1.knuts + m2.knuts) # complete me!
...
begin
	...
	function Compound(formula::String)
		...
		C = 0
		O = 0
		for m in eachmatch(r"(H|C|O)(\d*)", formula)
			molecule = m[1]
			amount = isempty(m[2]) ? 1 : parse(Int, m[2])
			if molecule == "H" 
				H += amount
			elseif molecule == "C"
				C += amount
...
Base.show(io::IO, compound::Compound) =
	print(io, "C$(compound.C)H$(compound.H)O$(compound.O)")
...
Base.isless(m1::WizCur, m2::WizCur) = 
	493galleons(m1)+29sickles(m1)+knuts(m1) < 493galleons(m2)+29sickles(m2)+knuts(m2)# complete me!
...
Base.isgreater(m1::WizCur, m2::WizCur) = isless(m2,m1)

# ╔═╡ 9226b2d8-5d94-11eb-2bea-491eb7dc1da7
Base.isequal(m1::WizCur, m2::WizCur) = !(isless(m1,m2) || isgreater(m1,m2))

# ╔═╡ a6e69762-43b4-4d6d-b5ae-57a585fcfb1c
max(money_harry,money_ron)
...
mass(c::Compound) = 1.008 * c.H + 12.011 * c.C + 15,999 * c.O
...
struct HSV <: MyColor
    ...
    function HSV(h::Real, s::Real, v::Real)
        # Hue wraps around 360
       	# s and v in [0, 1]
		return new(mod(h, 360), clamp(s, 0.0, 1.0), clamp(v, 0.0, 1.0))
    end
end
